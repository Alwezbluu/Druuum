...
def generate(tempo=120, length=2000, genre="punk"):
    """ Generates the midi file """
    #load the notes used to train the model
    with open(f'../drumGen/drums_generator/data/full_notes_{genre}', 'rb') as filepath:
...
def create_network(network_input, n_vocab, genre):
    ...
    model.add(LSTM(1024, return_sequences=True, recurrent_dropout=0,))
    model.add(LSTM(1024))
    model.add(BatchNorm())
    ...
    model.load_weights(f'../drumGen/drums_generator/weights/{genre}.hdf5')
